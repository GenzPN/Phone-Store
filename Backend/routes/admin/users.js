...
router.get('/all', async (req, res) => {
  try {
    const [users] = await db.promise().query('SELECT id, username, email, fullName, gender, image, isAdmin, created_at FROM Users');
    res.json(users);
  } catch (error) {
...
router.get('/:id', async (req, res) => {
  try {
    const [users] = await db.promise().query('SELECT id, username, email, fullName, gender, image, isAdmin, created_at FROM Users WHERE id = ?', [req.params.id]);
    if (users.length === 0) {
